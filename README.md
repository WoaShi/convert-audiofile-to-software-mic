...
- **运行环境**：需要安装 [.NET 10 Desktop Runtime（桌面运行时）](https://dotnet.microsoft.com/zh-cn/download/dotnet/10.0)。请根据您的系统架构下载对应安装包（通常 64 位系统选择 **x64** 版本）。
- **软件需求**：依赖虚拟音频驱动（如 VB-CABLE 或 VoiceMeeter）。软件会自动检测并绑定系统中的虚拟音频设备。
- **兼容性**：支持 Windows 10 / Windows 11，支持各类通过按住按钮发送语音的电脑端聊天软件。
- 如果2026-9-15版本让您感到困惑请下载2025-4-26版本。
